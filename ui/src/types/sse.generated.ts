...
export type ScenarioChoice = { scenarioId: string, label: string, executor: string, };

export type SseEvent = { "type": "data-session-id", data: DataSessionIdPayload, transient: boolean, } | { "type": "start", messageId: string, agentName: string, } | { "type": "step-start", stepId: string, messageId: string, agentName: string, } | { "type": "text-start", id: string, agentName: string, } | { "type": "text-delta", id: string, delta: string, } | { "type": "text-end", id: string, } | { "type": "tool-input-start", toolCallId: string, toolName: string, agentName: string, parallelBatchId: string | null, } | { "type": "tool-input-delta", toolCallId: string, inputTextDelta: string, } | { "type": "tool-input-available", toolCallId: string, toolName: string, input: unknown, parallelBatchId: string | null, } | { "type": "tool-output-available", toolCallId: string, output: string, } | { "type": "file", url: string, mediaType: string, } | { "type": "file-scenario-chips", messageId: string, uploadId: string, alternatives: Array<ScenarioChoice>, } | { "type": "rich-card" } & RichCardData | { "type": "clarify-needed", clarifyId: string, question: string, choices: Array<string>, timeoutMs: number, } | { "type": "tool-approval-needed", approvalId: string, toolName: string, toolInput: unknown, 
/**
