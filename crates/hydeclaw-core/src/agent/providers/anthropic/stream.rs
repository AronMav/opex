...
impl StreamingAnthropicUsage {
    pub(super) fn into_token_usage(self) -> Option<hydeclaw_types::TokenUsage> {
        if !self.seen {
            return None;
        }
...
pub(super) struct ThinkingState {
    ...
    pub(super) blocks: Vec<hydeclaw_types::ThinkingBlock>,
}
...
#[cfg(test)]
pub(super) fn parse_streaming_usage_for_test(lines: &[String]) -> Option<hydeclaw_types::TokenUsage> {
    let mut thinking = ThinkingState::default();
    let mut usage_buffer = StreamingAnthropicUsage::default();
