...
// `gateway` re-exports leaves from `hydeclaw-gateway-util` plus two
// crate-local leaves (`sse::coalescer`, `csp`). All consumed by
// `tests/integration_*.rs`. New `gateway::*` lib entries should live in
// `hydeclaw-gateway-util` rather than under `src/gateway/handlers/` so
...
// `db` re-exports leaves from `hydeclaw-db` plus three crate-local DB
// leaves (`curator_runs`, `access`, `upload_migration`). New DB modules
// should live in `hydeclaw-db` so this list shrinks over time.
pub mod db {
