...
Заменить строку счётчика массивов (иначе промпт внутренне рассинхронизован — 5 ключей, «four»):
```rust
...
Добавить в блок Categories (после строки `- events: ...`) — на английском, как остальные категории (правило «Write in the same language as the conversation» управляет ВЫВОДОМ, не языком инструкции):
```rust
...
```
(Примечание: маркер «Максимум 5» оставлен кириллицей намеренно — на него ассертит тест `extraction_prompt_enabled_has_open_items_disabled_does_not` из Step 1.)
...
**Interfaces:**
- Consumes: `PgPool`, `anyhow::Result` (alias в модуле), `use anyhow::Context` (уже импортирован — строка 1; `.context(...)` используется на строке 769).
