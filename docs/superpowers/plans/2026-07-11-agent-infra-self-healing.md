...
> **Актуализировано 2026-07-12** под текущий HEAD (влиты soul/initiative/decompose).
> Существенные изменения против первой редакции плана:
> 1. **Миграция 077 занята** (`077_agent_plans.sql`), следующая свободная — **080**.
> 2. **Telegram-доставка кнопок больше НЕ дыра/stretch.** Фича *initiative* уже
>    проложила путь отправки owner-gated inline-кнопок из core-контекста:
>    `crates/opex-core/src/agent/initiative/delivery.rs` даёт `resolve_owner_target(db,
>    agent, owner_id) -> Option<(channel, chat_id)>` и `send_proposal_to_channel(...)`.
>    Мы переиспользуем этот паттерн — Telegram-путь входит в v1.
> 3. **Образец callback теперь `handle_initiative_callback`** (`inline.rs:301`,
>    префиксы `iappr:`/`idismiss:`/`icancel:`) — ближе к нашей задаче, чем approval.
>    Цепочка перехвата в `reader.rs`: approval → initiative → clarify; наш infra-callback
>    вставляется после initiative.
> 4. **Новый слой — channels TS:** `channels/src/drivers/telegram.ts:1202` (`case
>    "initiative_proposal"`) — образец для рендера inline-кнопок; нужен `case
>    "infra_decision"` + строки локализации.
...
- **Миграции авто-применяются** на старте; следующий свободный номер — `080` (077-079 заняты agent_plans / initiative / goal).
...
**Создать:**
- `migrations/080_infra_decisions.sql` — таблица + UNIQUE partial index.
...
- `crates/opex-core/src/gateway/handlers/infra.rs` — endpoints + `resolve_infra_decision` + доставка кнопок.
...
- `crates/opex-core/src/gateway/handlers/channel_ws/inline.rs` — `handle_infra_callback` (образец — `handle_initiative_callback`).
- `crates/opex-core/src/gateway/handlers/channel_ws/reader.rs` — проводка callback (после initiative-callback).
- `channels/src/drivers/telegram.ts` — `case "infra_decision"` (кнопки `infra:ok:`/`infra:no:`).
- `channels/src/localization.ts` — строки `infraHeader`/`infraApprove`/`infraDismiss`.
...
- `crates/opex-core/src/agent/initiative/delivery.rs` — `resolve_owner_target(db, agent, owner_id)`
  + `send_proposal_to_channel(...)` как образец доставки owner-gated кнопок из core-контекста.
...
**Files:**
- Create: `migrations/080_infra_decisions.sql`
...
Создать `migrations/080_infra_decisions.sql`:
...
```bash
git add migrations/080_infra_decisions.sql crates/opex-db/src/infra_decisions.rs \
        crates/opex-db/src/lib.rs crates/opex-core/src/db/mod.rs \
        crates/opex-core/tests/integration_infra_decisions.rs
git commit -m "feat(infra): infra_decisions table + db module (m080)"
```
...
**Interfaces:**
- Consumes: `db::infra_decisions::{create, get, resolve_strict, mark_status, list}`; `notify(...)`; `AgentCore::base_engine`; `ChannelBus{ui_event_tx}`; `agent::initiative::delivery::resolve_owner_target(db, agent, owner_id)` + `ChannelActionRouter` (доставка Telegram-кнопок, образец — `send_proposal_to_channel`); `engine.cfg().agent.access.owner_id`; owner-check через `auth.access_guards` + `guard.is_owner(user_id)`.
- Produces:
...
async fn api_create_decision(
    ...
    if body.status == "pending" {
        ...
        deliver_infra_buttons(&infra.db, &agents, &channel_router, id, &body).await;
    }
    (axum::http::StatusCode::OK, Json(json!({"ok": true, "id": id.to_string()}))).into_response()
}
```

Добавить доставку кнопок (образец — `initiative/delivery.rs::{resolve_owner_target,
send_proposal_to_channel}` + TS `case "initiative_proposal"`):
...
async fn deliver_infra_buttons(
    db: &sqlx::PgPool,
    agents: &AgentCore,
    channel_router: &Option<crate::agent::channel_actions::ChannelActionRouter>,
    decision_id: Uuid,
    body: &CreateDecisionBody,
) {
    let Some(router) = channel_router.as_ref() else { return };
    let Some(engine) = agents.base_engine().await else { return };
    let agent_name = engine.cfg().agent.name.clone();
    let owner_id = engine.cfg().agent.access.owner_id.clone();
...
Примечание для исполнителя: `api_create_decision` теперь дополнительно берёт
`State(agents): State<AgentCore>` и доступ к `channel_router`. Точное поле роутера
в `AppState` — сверить с тем, как его достаёт initiative `tick.rs` (обычно
`state.channels.channel_router` или подобное через `ChannelBus`); `owner_id` путь —
`engine.cfg().agent.access.owner_id: Option<String>` (подтверждено в
`initiative/delivery.rs` doc-комментарии).

Остальные роуты create-группы (list, patch) — тем же блоком:
...
## Task 6: Telegram-путь подтверждения — `infra:ok/no` callback + TS-адаптер

**Files:**
- Modify: `crates/opex-core/src/gateway/handlers/channel_ws/inline.rs` (функция `handle_infra_callback`)
- Modify: `crates/opex-core/src/gateway/handlers/channel_ws/reader.rs:122` (проводка после initiative)
- Modify: `channels/src/drivers/telegram.ts:1202` (`case "infra_decision"`)
- Modify: `channels/src/localization.ts` (строки `infra*`)
- Test: source-scan wiring test в `reader.rs` (образец — `approval_wired_before_clarify`)

**Interfaces:**
- Consumes: `resolve_infra_decision` (Task 5, `pub(crate)`); owner-check через `ctx.auth.access_guards` + `guard.is_owner(user_id)`; `ctx.infra`, `ctx.agents` (поля `CwsCtx`, подтверждены в `channel_ws/types.rs:26`).
- Produces: `async fn handle_infra_callback(ctx, engine, agent_name, request_id, msg, out_tx) -> bool` — перехватывает `infra:ok:UUID` / `infra:no:UUID` (сигнатура один-в-один с `handle_initiative_callback`).

**Примечание:** доставка кнопок владельцу реализована в Task 5 (`deliver_infra_buttons`) через паттерн initiative — Telegram-путь входит в v1. Эта задача добавляет ПРИЁМ callback (Rust) и РЕНДЕР кнопок (TS-адаптер).

- [ ] **Step 1: Реализовать `handle_infra_callback` (образец — `handle_initiative_callback`, `inline.rs:301`)**

В `inline.rs` рядом с `handle_initiative_callback` добавить:

```rust
/// Перехват Telegram inline-callback `infra:ok:UUID` / `infra:no:UUID`. Owner-only.
/// Сигнатура и owner-gate копируют handle_initiative_callback.
pub(super) async fn handle_infra_callback(
    ...
    let is_callback = msg.context.get("is_callback")
    ...
    let (rest, approved) = if let Some(r) = text.strip_prefix("infra:ok:") {
        ...
        return false; // не наш callback — пусть пробуют другие перехватчики
    };

    let user_id = msg.user_id.clone();
    // Security: только владелец. Fail-closed если guard отсутствует.
    ...
    if !is_owner {
        ...
        let _ = out_tx.send(OutboundMsg::Wire(ChannelOutbound::Error {
    ...
    let Ok(id) = rest.parse::<uuid::Uuid>() else { return true; };
    match crate::gateway::handlers::infra::resolve_infra_decision(
        ...
        Ok(()) => {
            ...
            let _ = out_tx.send(OutboundMsg::Wire(ChannelOutbound::Done {
                request_id: request_id.to_string(),
                text: text.to_string(),
            })).await;
        }
...
- [ ] **Step 2: Проводка в reader.rs (после initiative-callback, `:122`)**

В `reader.rs` после блока `handle_initiative_callback` (строки 122-126) добавить:
...
В `reader.rs` в `#[cfg(test)] mod tests` добавить:
...
- [ ] **Step 5: TS-адаптер — рендер кнопок (образец — `case "initiative_proposal"`, `telegram.ts:1202`)**

В `channels/src/drivers/telegram.ts` в `switch` по `action.name` добавить кейс:

```ts
case "infra_decision": {
  const decisionId = params?.decision_id as string;
  const container = (params?.container as string) ?? "";
  const proposed = (params?.proposed_action as string) ?? "";
  ...
  await bot.api.sendMessage(chatId, body, { reply_markup: keyboard });
  break;
}
```

Точные имена (`InlineKeyboard`, `bot.api.sendMessage`, `chatId`, `params`, `strings`)
скопировать из соседнего `case "initiative_proposal"`.

В `channels/src/localization.ts` добавить в интерфейс строк и в оба словаря (ru/en):
...
infraDismiss: string;
```
ru: `infraHeader: "🛠 Требуется решение по инфраструктуре", infraApprove: "✅ Выполнить", infraDismiss: "❌ Отклонить"`.
en: `infraHeader: "🛠 Infra decision needed", infraApprove: "✅ Apply", infraDismiss: "❌ Dismiss"`.

- [ ] **Step 6: Сборка channels + Commit**

Run: `cd channels && bun test` (если есть тесты) и `bunx tsc --noEmit` (проверка типов).
...
git add crates/opex-core/src/gateway/handlers/channel_ws/inline.rs \
        ...
        channels/src/drivers/telegram.ts channels/src/localization.ts
git commit -m "feat(infra): Telegram infra:ok/no callback + adapter buttons → resolve_infra_decision"
```
...
Run: `make remote-deploy` (собирает core+watchdog+memory-worker на сервере, свопит, рестартит). Затем **вручную** задеплоить не-Rust артефакты (deploy свопит только Rust-бинарники):
- `config/skills/infra-triage.md` → `~/opex/config/skills/`;
- `channels/src/{drivers/telegram.ts,localization.ts}` → пересобрать/перезапустить channels (`POST /api/services/channels/restart`);
- UI: `deploy-ui.sh` (union-swap `_next/static`).

Миграция 080 применяется автоматически при рестарте core.
...
Записать итог в `MEMORY.md` (новый файл `project_infra_self_healing.md`): что задеплоено, готчи (config/skills + channels копировать руками, миграция 080, infra-event идёт мимо alert_events, доставка кнопок переиспользует initiative/delivery).
...
- Компонент 5 (исполнение одобренного, единая resolve, 2 входа) → Task 5 (resolve_infra_decision + UI-вход + доставка Telegram-кнопок через initiative/delivery) + Task 6 (Telegram-приём + TS-адаптер) + Task 8 (UI-кнопки). Telegram-путь входит в v1 (не stretch — доставка кнопок решена переиспользованием). ✔
...
- Поле `channel_router` в `AppState` (Task 5 `deliver_infra_buttons`) — сверить с тем, как его достаёт `initiative/tick.rs`. `CwsCtx` поля (`infra`/`agents`/`auth`) подтверждены (`channel_ws/types.rs:26`); `OutboundMsg::Wire(ChannelOutbound::{Done,Error})` — как в `handle_initiative_callback`.
- `engine.cfg().agent.access.owner_id` путь — подтверждён в `initiative/delivery.rs` (H1 security).
- Точный helper запроса в ui queries (`apiFetch` vs `api.*`).
- TS: имена `InlineKeyboard`/`bot.api.sendMessage`/`params`/`strings` — из соседнего `case "initiative_proposal"` (`telegram.ts:1202`).

**Актуализация 2026-07-12 (изменения кода):** миграция 077→080; Telegram-доставка кнопок переведена из «дыра/stretch» в v1 через переиспользование `initiative/delivery.rs`; образец callback сменён на `handle_initiative_callback`; добавлен слой channels TS (Task 6, шаг 5). Остальные компоненты не затронуты изменениями HEAD.
