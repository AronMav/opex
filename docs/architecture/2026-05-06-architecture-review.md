...
- **E2E / chaos:** [test-pi-e2e.py](../../test-pi-e2e.py), [test-pi-chaos.py](../../test-pi-chaos.py), [test-pi-concurrency.py](../../test-pi-concurrency.py), [test-pi-trace-correlation.py](../../test-pi-trace-correlation.py) all run against a real Pi at `192.168.1.82:18789`. The chaos test simulates random mid-stream SSE drops and asserts `Last-Event-ID` resume produces a deduplicated event timeline — this is the right way to validate streaming reliability.
...
9. **Chaos and concurrency tests against real Pi.** [test-pi-chaos.py](../../test-pi-chaos.py) exercises the SSE `Last-Event-ID` resume by simulating mid-stream drops. [test-pi-concurrency.py](../../test-pi-concurrency.py) and [test-pi-trace-correlation.py](../../test-pi-trace-correlation.py) cover other axes. Most projects only test the happy path.
