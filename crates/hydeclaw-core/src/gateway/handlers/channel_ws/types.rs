...
//!   single-writer task. Wraps [`hydeclaw_types::ChannelOutbound`] plus
...
use hydeclaw_types::ChannelOutbound;
...
///
/// Computed via `hydeclaw_db::sessions::dm_scope_keys` so the key matches
...
impl SessionKey {
    pub fn from_inbound(agent_name: &str, user_id: &str, channel: &str, dm_scope: &str) -> Self {
        let (eff_user, eff_channel) =
            hydeclaw_db::sessions::dm_scope_keys(user_id, channel, dm_scope);
        Self {
