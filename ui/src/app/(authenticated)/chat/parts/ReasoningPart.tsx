  ...
  return (
    ...
    >
      <CollapsibleTrigger asChild>
        ...
        >
          <div className={`h-1.5 w-1.5 rounded-full bg-primary/80 ${streaming ? "animate-pulse" : ""}`} />
          <span className="font-mono text-xs font-semibold uppercase tracking-wider text-primary/70 dark:text-primary/50">
            {t("chat.reasoning")}
          </span>
          <ChevronRight className="ml-auto h-3.5 w-3.5 text-primary/40 transition-transform duration-200 group-data-[state=open]:rotate-90" />
        </button>
      </CollapsibleTrigger>
      <CollapsibleContent>
        <MessageContent
