...
**Спека:** [docs/superpowers/specs/2026-07-09-agent-soul-foundation-design.md](../specs/2026-07-09-agent-soul-foundation-design.md) (ревизия 3). При расхождении план ↔ спека — спека главнее.
...
**Interfaces:**
- Produces: колонки `memory_chunks.kind TEXT NOT NULL DEFAULT 'fact'`, `importance REAL NOT NULL DEFAULT 5.0`, `lineage UUID[]`; инвариант «hard-delete только kind='fact'».
...
В `crates/opex-memory-worker/src/handlers/reindex.rs:112` (trailing DELETE):
...
К последнему добавить строку в существующий комментарий над DELETE: `// kind='fact' guard: reindex re-populates FILE-backed chunks only; soul biography (event/reflection) must survive clear_existing (spec §1, rev3 blocker).`
...
mod soul_guard_tests {
    ...
    async fn reindex_clear_existing_spares_soul_kinds(db: sqlx::PgPool) {
        ...
        sqlx::query(
            "DELETE FROM memory_chunks WHERE agent_id = $1 AND created_at < $2 AND kind = 'fact'",
        )
        .bind("A").bind(cutoff)
        .execute(&db).await.unwrap();
...
```bash
git add migrations/076_memory_soul_columns.sql crates/opex-core/src/scheduler/mod.rs crates/opex-memory-worker/src/handlers/reindex.rs
git commit -m "feat(soul): m076 kind/importance/lineage + exempt biography from all three hard-delete paths"
```
...
`insert_chunk_inner` + оба врапера: добавить в конец параметров `kind: &str, importance: f32, lineage: Option<&[uuid::Uuid]>`, биндить `$9=kind, $10=importance, $11=lineage`. Существующие вызовы в тестах этого файла и в `opex-core/src/memory/store.rs` (3 места: `index`, `reindex_source`, `index_batch`) дополняются `, "fact", 5.0, None` — store правится в Task 3, здесь только opex-db компилируется (его собственные тесты `test_insert_chunk_*` дополнить теми же тремя аргументами).
...
- Modify: `crates/opex-core/src/memory/mod.rs` (объявить `pub mod soul;`, re-export `pub use crate::db::memory_queries::SoulCandidate;` рядом с существующими re-export'ами MemoryChunk/MemoryResult)
...
`handle_memory_delete` — сменить сигнатуру на `pub async fn handle_memory_delete(memory_store: &dyn MemoryService, db: &sqlx::PgPool, args: &serde_json::Value) -> String` (вызов находится в `agent/tool_executor.rs` или `pipeline/handlers` — найти по `handle_memory_delete(` и добавить `db`-аргумент, `PgPool` там доступен через deps/engine cfg):

```rust
    // Spec §5.2: agents must not be able to purge their own biography.
...
`api_patch_memory` — до применения обновлений (после early-валидации):

```rust
    // Spec §5.2: UI patch must not rewrite biography chunks.
    let kind: Option<String> = sqlx::query_scalar("SELECT kind FROM memory_chunks WHERE id = $1")
        .bind(id)
...
```bash
git add crates/opex-core/src/agent/pipeline/memory.rs crates/opex-core/src/gateway/handlers/memory.rs crates/opex-core/src/agent/tool_executor.rs
git commit -m "feat(soul): spoofing guards — reserved soul_* source, kind-guarded delete/patch"
```
...
**⏸ Батч 1 (сервер):** после этого таска прогнать на сервере `make test-db` + `cargo test -p opex-db` (изолированный worktree). Ожидаемо зелено; починить до перехода к Task 6.
...
Промпт (`extract_and_save_inner`, шаг 4) — заменить целиком; диалог во fencing, добавлена категория events и правило игнорирования внутридиалоговых команд:
...
- Modify: все вызовы `ensure_workspace_scaffold(` (найти `Grep "ensure_workspace_scaffold("` — минимум `agent/engine/lifecycle.rs:281`) — новый параметр `soul_enabled: bool`
...
mod tests {
    ...
    fn render_reserializes_only_whitelisted_bullets_inside_framing() {
        ...
        assert!(!block.contains("</self_portrait>"), "frame-breaking token must be stripped");
...
Все вызовы `ensure_workspace_scaffold(` дополнить `cfg.agent.soul.enabled` (или `false`, если в точке вызова агент-конфига нет — проверить каждый call-site по grep).
...
```bash
git add crates/opex-core/src/agent/soul/ crates/opex-core/src/agent/workspace.rs crates/opex-core/src/agent/engine/lifecycle.rs
...
/// In-core per-agent runtime state: reflection lock + failure backoff.
...
struct SoulRuntime {
    ...
    backoff: dashmap::DashMap<String, (u32, Option<chrono::DateTime<chrono::Utc>>)>,
}

static SOUL_RUNTIME: std::sync::LazyLock<SoulRuntime> = std::sync::LazyLock::new(|| SoulRuntime {
...
pub async fn maybe_reflect(
    ...
    // backoff pause
    if let Some(entry) = SOUL_RUNTIME.backoff.get(agent)
        && let (_, Some(until)) = *entry
        && chrono::Utc::now() < until
    {
        return;
    }
    let lock = SOUL_RUNTIME.locks.entry(agent.to_string())
    ...
    match run_cycle(db, agent, provider, memory_store, deps).await {
        Ok(()) => {
            SOUL_RUNTIME.backoff.remove(agent);
            tracing::info!(agent, "reflection cycle complete");
        }
        Err(e) => {
            tracing::warn!(agent, error = %e, "reflection cycle failed");
            let mut entry = SOUL_RUNTIME.backoff.entry(agent.to_string()).or_insert((0, None));
            entry.0 += 1;
            if entry.0 >= BACKOFF_AFTER_FAILURES {
                entry.1 = Some(chrono::Utc::now() + chrono::Duration::hours(BACKOFF_PAUSE_HOURS));
                entry.0 = 0;
                let _ = crate::gateway::handlers::notifications::notify(
                    db, &deps.ui_event_tx, "agent_error",
...
async fn run_cycle(
    ...
    // Exact record_tool_execution signature: crates/opex-db/src/tool_audit.rs:11 —
    // adapt argument list to it verbatim.
    let _ = crate::db::tool_audit::record_tool_execution(
        db, agent, None, "soul_reflection",
        serde_json::json!({"insights": inserts.len(), "self_updates": applied}),
        "applied", None, None,
    ).await;
    Ok(())
}
...
async fn update_self_md(
    ...
    // checkpoint BEFORE the write (spec §5.5); same helper the workspace tools use
    if let Some(cm) = &deps.checkpoint {
        if let Err(e) = cm.ensure_checkpoint(agent, &deps.workspace_dir).await {
            tracing::warn!(agent, error = %e, "pre-SELF.md checkpoint failed (write continues)");
        }
    }
...
1. Аудит-строка пишется в конце `run_cycle` (код выше); точную сигнатуру `record_tool_execution` сверить по `crates/opex-db/src/tool_audit.rs:11` и подогнать аргументы (session_id=None, duration/error=None).
2. `ensure_checkpoint` — `pub(crate)` в `checkpoint_manager.rs:260`; если сигнатура отличается (`(agent, workspace_dir)`), подогнать вызов; альтернатива — `maybe_checkpoint` из `tool_handlers/workspace.rs:55`.
3. `dashmap` и `LazyLock`: dashmap уже в дереве (session_locks); `std::sync::LazyLock` доступен в 2024 edition.
4. Вызов из экстрактора — последняя строка `extract_and_save_inner` (после save_events): `crate::agent::soul::reflection::maybe_reflect(db, agent_name, provider, memory_store, soul_deps).await;` — значит `extract_and_save`/`extract_and_save_inner` принимают `soul_deps: &crate::agent::soul::reflection::SoulDeps` ВМЕСТО голого `SoulConfig` из Task 7 — отрефакторить Task 7-сигнатуру здесь (`soul.enabled` → `soul_deps.cfg.enabled` и т.д.), `spawn_knowledge_extraction` собирает `SoulDeps` из engine cfg/state (checkpoint_manager и ui_event_tx достать так же, как их достаёт FinalizeCtx-конструктор на finalize.rs:695-712).

- [ ] **Step 3:** Run: `cargo check --all-targets` → чисто.

- [ ] **Step 4: sqlx-тест конкурентности (сервер, батч 2)** — linux-gated, рядом с reflection.rs тестами: два `tokio::spawn(maybe_reflect(...))` с MockProvider (взять существующий мок LLM-провайдера — grep `MockProvider`/`FakeProvider` в тестах engine; если пригодного нет — тестировать `try_lock`-скип напрямую: первый таск держит лок, второй `maybe_reflect` возвращается мгновенно, счётчик LLM-вызовов мока == вызовам одного цикла).
...
В `build()` после L0-блока (`:493`, после `let memory_len = ...; let pre_todo_len = ...` — вставить МЕЖДУ ними):

```rust
        // Soul: SELF portrait + L1 biography (spec §4/§6)
        let pre_soul_len = system_prompt.len();
        ...
        let soul_len = system_prompt.len() - pre_soul_len;
```

и в конструкцию breakdown (найти место, где заполняются `memory: memory_len` и т.п.) добавить `soul: soul_len`.
...
- [ ] **Step 1:** Найти DTO: `Grep "MemoryDocumentDto|struct.*Memory.*Dto" crates/opex-core --type rust`. Добавить в struct:
...
и заполнить в list-эндпоинте из строк БД (SELECT list-эндпоинта дополнить колонками `kind, importance`, если он свой, а не через memory_queries).

- [ ] **Step 2:** Регенерация биндингов: найти export-тест `Grep "export_bindings|#\[ts\(export" crates/opex-core` → выполнить его на сервере вместе с батчем ИЛИ локально `cargo test -p opex-core export_bindings -- --ignored` если Windows позволяет чисто-генерационный тест (если падает — регенерировать на сервере, забрать файл scp/commit оттуда). CI гоняет gen-types drift-чек — незакоммиченный биндинг уронит CI.
...
### Task 12: Серверные тесты, деплой, E2E

**Files:** нет новых — деплой и ручная проверка.
...
4. Попросить агента: изменить SELF.md (`workspace_write`) → отказ; переименовать → отказ; `memory(index, source="soul_event:x")` → отказ; `memory(delete)` на event-чанке → отказ.
...
| Спека | Таск |
| --- | --- |
| §1 миграция, 3 hard-delete пути, soul-ретривал, kind-фильтры, диверсификация | 1, 2, 3 |
...
| §5 спуфинг-инварианты, is_read_only, rename, checkpoint/audit, карантин | 4, 8, 9 (карантин = runbook: SQL из спеки §5.6, кодировать нечего) |
