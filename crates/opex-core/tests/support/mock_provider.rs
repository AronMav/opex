...
use opex_core::opex_types::{LlmResponse, Message, ToolCall, ToolDefinition};
use std::sync::Mutex;
use tokio::sync::mpsc;
    ...
    pub fn tool_call(
        ...
        Self {
            ...
            tool_calls: vec![ToolCall {
                id: opex_types::ids::ToolCallId::from(id.into()),
                name: name.into(),
                arguments,
...
impl MockLlmProvider for MockProvider {
    async fn chat(
        ...
        // NOTE: `opex_types::LlmResponse` does NOT implement `Default`, so every
        // field must be listed explicitly. Keep this aligned with the struct shape in
        // `crates/opex-types/src/lib.rs`. If a new field is added there, add it
