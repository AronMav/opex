# OPEX Configuration Reference

Полный справочник по всем файлам конфигурации, переменным окружения и форматам данных OPEX.
...
3. [config/opex.toml — главный конфиг](#3-configopextoml--главный-конфиг)
...
| `config/opex.toml` | Всё остальное: сервер, лимиты, процессы, Docker | Да (через `notify`-крейт) |
...
**При первом запуске** `.env` создаётся автоматически с генерацией `OPEX_AUTH_TOKEN` и `OPEX_MASTER_KEY` через `rand`. `DATABASE_URL` нужно добавить вручную.
...
## 3. config/opex.toml — главный конфиг

Загружается при старте через `AppConfig::load()`. Путь передаётся как первый аргумент CLI или берётся из дефолта `config/opex.toml`.
...
| `auth_token_env` | Option\<String\> | — | Имя env-переменной с auth-токеном (обычно `"OPEX_AUTH_TOKEN"`) |
...
> Memory worker — отдельный бинарник (`opex-memory-worker`). Режим `listen` использует `PgListener` на канале `memory_tasks_new` (migration 023), опрос остаётся как safety net.
...
| `service_name` | String | `"opex-core"` | Имя сервиса, передаваемое в коллектор |
...
Секреты шифруются ChaCha20-Poly1305 (мастер-ключ из `OPEX_MASTER_KEY`) и хранятся в таблице `secrets`.
...
| `OPEX_AUTH_TOKEN` / `AUTH_TOKEN` | — | Bearer-токен для аутентификации в Core |
...
| Ресурс | Путь |
|--------|------|
| Core binary | `~/opex/opex-core-aarch64` |
| Watchdog binary | `~/opex/opex-watchdog-aarch64` |
| Memory worker binary | `~/opex/opex-memory-worker-aarch64` |
| Config | `~/opex/config/` |
| Workspace | `~/opex/workspace/` |
| UI static | `~/opex/ui/out/` |
| Migrations | `~/opex/migrations/` |
| Docker configs | `~/opex/docker/` |
| Toolgate source | `~/opex/toolgate/` |
| `.env` | `~/opex/.env` (рядом с бинарником) |
...
scp toolgate/changed_file.py user@pi:~/opex/toolgate/
...
```ini
[Service]
ExecStart=/home/user/opex/opex-core-aarch64 config/opex.toml
...
*Документ сгенерирован на основе исходного кода: `crates/opex-core/src/config/mod.rs`, `crates/opex-core/src/memory/mod.rs`, `crates/opex-core/src/secrets.rs`, `crates/opex-core/src/tools/yaml_tools.rs`, `crates/opex-memory-worker/src/config.rs`, `config/opex.toml`, `toolgate/config.py`.*
