# Полное переименование HydeClaw → OPEX
...
Полностью уйти от имени `hydeclaw` во всём проекте и заменить его на `opex`,
сделав русский язык основным. Критерий успеха — `grep -rni hydeclaw` по
...
| БД | Переименовываем (`hydeclaw` → `opex`: имя БД, роль, пароль) |
| GitHub-репо | Переименовываем `AronMav/hydeclaw` → `AronMav/opex` (ручная операция, авто-редирект) |
...
| Было | Стало |
|---|---|
| крейты `hydeclaw-core/-types/-watchdog/-memory-worker/-db/-embedding/-gateway-util` | `opex-core/-types/-watchdog/-memory-worker/-db/-embedding/-gateway-util` |
| импорты `hydeclaw_core::`, `hydeclaw_types::` … | `opex_core::`, `opex_types::` … |
| бинарники `hydeclaw-core-x86_64` и т.д. | `opex-core-x86_64` и т.д. |
| RUST_LOG/OTEL `hydeclaw_core`, `hydeclaw-memory-worker`, service.name | `opex_core`, `opex-memory-worker` |
| insta-снапшоты `hydeclaw_core__…snap` | `opex_core__…snap` (переименовать файлы) |
| `config/hydeclaw.toml` | `config/opex.toml` |
| `config/watchdog.toml` (имена сервисов внутри) | обновить ссылки |
| mDNS-хостнейм `hydeclaw.local` (хардкод в `network.rs`) | `opex.local` |

### Рантайм / env (PR1 — dual-read, PR2 — миграция на сервере)

| Было | Стало |
|---|---|
| `HYDECLAW_AUTH_TOKEN`, `HYDECLAW_MASTER_KEY` | `OPEX_AUTH_TOKEN`, `OPEX_MASTER_KEY` (значения 1:1!) |
| `HYDECLAW_CORE_URL`, `HYDECLAW_DISABLE_REDRIVE` | `OPEX_CORE_URL`, `OPEX_DISABLE_REDRIVE` |
| test-only `HYDECLAW_PG_TEST_IMAGE`, `…_GEMINI_TEST_*`, `…_MIGRATION_BUDGET_MS`, `…_OAUTH_CREDENTIALS_PATH`, `…_GEMINI_TEST_TOKEN_ENDPOINT` | `OPEX_*` (без fallback — переименовать сразу) |

### Клиент / localStorage (PR1)

| Было | Стало | Примечание |
|---|---|---|
| `hydeclaw.auth.token` | `opex.auth.token` | **+ shim**: читать старый ключ, если нового нет (не разлогинит) |
| `hydeclaw.language` | `opex.language` | **+ shim** (не сбросит выбранный язык) |
| `hydeclaw.draft.*`, `hydeclaw.chat.lastSession`, `hydeclaw.lastSession`, `hydeclaw-rq` (IndexedDB), `hydeclaw_wizard_progress`, `hydeclaw_debug_sse`, событие `hydeclaw:stop-stream` | `opex.*` / `opex-rq` / `opex:stop-stream` | без shim — эфемерное, допустим сброс |

### Инфраструктура (PR2 — runbook на сервере)

| Было | Стало |
|---|---|
| `~/hydeclaw` (install), `~/hydeclaw-src` (build) | `~/opex`, `~/opex-src` |
| systemd `hydeclaw-core/-watchdog/-memory-worker.service` | `opex-core/-watchdog/-memory-worker.service` |
| `.deploy.env` `SERVER_DIR=~/hydeclaw` | `~/opex` |
| docker-образы `hydeclaw-pg`, `hydeclaw-tts-silero`, `hydeclaw-mcp-*` | `opex-pg`, `opex-tts-silero`, `opex-mcp-*` (retag/пересборка на сервере) |
| docker-сеть `hydeclaw`, volume-маунт `~/hydeclaw/workspace` | сеть `opex`, `~/opex/workspace` |
| `POSTGRES_DB=hydeclaw`, `pg_isready -U hydeclaw`, роль/пароль `hydeclaw` | `opex` (на живой БД через `ALTER DATABASE/ROLE … RENAME` + `ALTER ROLE … PASSWORD`) |
| `DATABASE_URL=…hydeclaw:hydeclaw@…/hydeclaw` | `…opex:opex@…/opex` |

---

## Капкан миграций БД и его решение

Слово `hydeclaw` в `migrations/*.sql` встречается двух видов:

1. **Комментарии** (`-- HydeClaw — consolidated schema` в 001, 022, 029, 033, и др.).
2. **Функциональный тег `@hydeclaw:ephemeral`** (m030, m050) — живая метка в
   каталоге БД: `COMMENT ON TABLE … IS '@hydeclaw:ephemeral …'`; код бэкапа
   (`gateway/handlers/backup.rs`) ищет ephemeral-таблицы по
   `LIKE '@hydeclaw:ephemeral%'`.
...
1. Редактируем `hydeclaw`→`opex` во **всех** `.sql`, включая комментарии старых
...
2. Новая миграция `051_rename_ephemeral_tag.sql` переписывает живые
   `COMMENT ON TABLE … @hydeclaw:ephemeral` → `@opex:ephemeral` для всех
...
Итог: `grep -rni hydeclaw` пуст, прод-БД стартует без ошибок.
...
**старой** инфраструктуре (старый `.env` с `HYDECLAW_*`, старые пути
`~/hydeclaw`) — деплой безопасен.

**Объём:**

1. **Крейты:** переименовать 7 директорий `crates/hydeclaw-*` → `crates/opex-*`;
   `[package] name`, workspace `members`, path-deps (`opex-types = { path = … }`);
   все `use hydeclaw_*::` → `use opex_*::`; имена бинарников; RUST_LOG/OTEL-строки;
   insta-снапшоты (`hydeclaw_core__…` → `opex_core__…`).
2. **Env — dual-read:** ввести хелпер `env_var("AUTH_TOKEN")`, читающий
   `OPEX_AUTH_TOKEN`, при отсутствии — `HYDECLAW_AUTH_TOKEN`. Применить к
   ...
   fallback.
3. **Конфиг:** `config/hydeclaw.toml` → `config/opex.toml`; загрузчик в
...
`deploy/server/*.service`, `docker/hydeclaw-core.service`,
...
| # | Шаг | Гард / откат |
|---|---|---|
| 0 | `pg_dump` БД + `tar` `~/hydeclaw` + копия `.env` в `~/opex-migration-backup/` | при ошибке — restore из бэкапа |
| 1 | Стоп `hydeclaw-core/-watchdog/-memory-worker` | `start` старых юнитов |
| 2 | В `.env`: ключи `HYDECLAW_*`→`OPEX_*`, **значения 1:1** (особенно `MASTER_KEY`) | dual-read из PR1 страхует частичную правку |
| 3 | `mv ~/hydeclaw ~/opex`, `mv ~/hydeclaw-src ~/opex-src` | `mv` обратно |
| 4 | БД: `docker exec psql -d postgres` → `ALTER DATABASE hydeclaw RENAME TO opex; ALTER ROLE hydeclaw RENAME TO opex; ALTER ROLE opex PASSWORD 'opex';` → правка `DATABASE_URL` | `ALTER … RENAME` обратимы; **прежде проверить, что pg-data — сохраняемый volume, а не пересоздаётся при смене имени проекта** |
...
| 6 | Docker: `docker tag hydeclaw-*→opex-*` (pg, tts-silero, mcp-*), правка compose (`name: opex`, volume `~/opex/workspace`), `docker compose up -d` | retag обратим; **критично не потерять pg-data-volume** |
...
3. Снести старые `hydeclaw-*.service` с сервера и `~/opex-migration-backup/`
   (после подтверждения).
4. Финальный аудит: `grep -rni hydeclaw` по репо = **0**.
...
- **PR3:** `grep -rni hydeclaw` = 0; `make doctor` 16/16 после удаления fallback.
...
- **PR3** — revert; требует возврата `HYDECLAW_*` ключей на сервере (поэтому PR3
  только после стабилизации).

## Критерии готовности

1. `grep -rni hydeclaw` по репозиторию = 0.
