...
| | |
| --- | --- |
| **Rust-ядро** | Ядро написано на Rust (только rustls, без OpenSSL), без Node и Python на горячем пути. Три Rust-сервиса — ядро, watchdog, memory worker — плюс PostgreSQL 17 + pgvector; адаптеры каналов (Bun) и медиа-хаб (Python) работают как управляемые дочерние процессы, Docker хостит песочницу и MCP-контейнеры. |
| **Безопасность включена по умолчанию** | Не плагины и не флаги конфига — всё в коробке и включено: vault с аутентифицированным шифрованием (ChaCha20-Poly1305), вычищающий креды из конфигов; SSRF-блокировка на уровне DNS-резолвера (иммунна к DNS-rebinding — это не проверка строки URL); PII-редакция по умолчанию — редактирует, а не блокирует; provenance-маркировка недоверенного контента; Docker-песочница; deny-first политика инструментов. Полная модель угроз — ниже. |
| **Эксплуатация из коробки** | Отдельный watchdog-бинарник напишет вам в ваш же чат-канал, если агент завис или процесс упал. Самовосстанавливающийся супервизор процессов, `/api/doctor` с 15 проверками здоровья (включая сканирование workspace на утёкшие креды), бэкапы `pg_dump` в один клик. |
| **Каталог моделей, который не устаревает** | Показывать стоимость умеют многие — но цифры обычно берутся из зашитого в репозиторий прайс-файла, который отстаёт от реальности. OPEX определяет контекстные окна, цены и возможности тысяч моделей из живого внешнего каталога (models.dev ∪ OpenRouter): $-учёт по сессиям с отдельным трекингом cache- и reasoning-токенов, пресеты провайдеров с автозаполнением URL / типа / списка моделей, параметры запросов гейтятся по фактическим возможностям модели. |
| **Память, которую можно прочитать** | Долгосрочная память агента — Markdown-файлы: редактируются руками, дружат с git, являются источником истины. Редкость — индекс за ними: pgvector + полнотекст + триграммы параллельно в PostgreSQL, слитые RRF- и MMR-ранжированием, автоматическая синхронизация из файлов, два уровня — сырой с временным затуханием и закреплённый постоянный. |
| **Всё — файл** | Личности и навыки — Markdown, инструменты — YAML (авторизация из vault, JSONPath-трансформации, импорт из OpenAPI), всё с hot-reload. Встроенный Куратор по расписанию архивирует устаревшие навыки и чинит сломанные. |
| **Работает без присмотра** | Cron с часовыми поясами и джиттером и heartbeat'ы агентов — это база; отличие — goal loop: отдельный проход LLM-судьи проверяет, достигнута ли поставленная цель на самом деле, а не «модель перестала вызывать инструменты». Результаты уходят в любой канал; человек остаётся в контуре: подтверждения (обратный отсчёт, редактируемые аргументы) и инструмент `clarify` для вопросов посреди выполнения. |
| **Флот, а не бот** | Агенты в общей сессии — всегда живые пиры: вы (или другой агент) можете писать им, опрашивать и останавливать посреди разговора (ask / status / kill), а не запускать одноразовые сабагентские прогоны. Маршрутизация @-упоминаниями, жёсткий denylist инструментов для субагентов без рекурсивного спавна, shadow-git-чекпоинты рабочих папок с `/rollback`. |
...
| --- | --- |
| **Каналы** | Telegram, Discord, Slack, Matrix, IRC, WhatsApp, Email — один процесс-адаптер, коды сопряжения и allowlist'ы, голосовые режимы по чатам; slash-команды из единого реестра — автодополнение в web, нативное меню команд в Telegram, локализованный `/help`, файл-обработчики доступны как команды |
...
| **Стандарты** | MCP-серверы как on-demand Docker-контейнеры, импорт инструментов из OpenAPI, OpenAI-совместимый `/v1` API, LSP-интеллект для агентов (Python через pyright) |
