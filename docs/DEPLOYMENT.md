...
HydeClaw — единый Rust-бинарник, который запускает HTTP API, агентов, LLM-вызовы,
...
systemd: hydeclaw-watchdog (Rust) — health monitoring + alerting
...
```bash
cargo build --release
# бинарники: target/release/hydeclaw-{core,watchdog,memory-worker}
```
...
# бинарники: target/aarch64-unknown-linux-gnu/release/hydeclaw-{core,watchdog,memory-worker}
```
...
```
hydeclaw/
├── hydeclaw-core-aarch64        # Rust binary (ARM64)
├── hydeclaw-core-x86_64         # Rust binary (x86_64)
├── hydeclaw-watchdog-aarch64
├── hydeclaw-watchdog-x86_64
├── hydeclaw-memory-worker-aarch64
├── hydeclaw-memory-worker-x86_64
├── hydeclaw-ui.tar.gz           # pre-built Next.js static UI
├── config/                      # default agent configs, hydeclaw.toml
...
собирает через zigbuild, публикует `hydeclaw-v*.tar.gz` в GitHub Releases.
...
7. Собирает Docker-образы: `hydeclaw-pg:17-age-pgvector`, `browser-renderer`, `hydeclaw-sandbox`
...
> **ВАЖНО:** `HYDECLAW_MASTER_KEY` шифрует secrets vault (ChaCha20-Poly1305).
...
scp toolgate/providers/tts.py aronmav@192.168.1.82:~/hydeclaw/toolgate/providers/
...
**Не нужно:** перезапуск hydeclaw-core — Core перезапускает toolgate как child process.
...
ExecStart=/home/user/hydeclaw/hydeclaw-core-aarch64
...
Type=notify
WorkingDirectory=/home/user/hydeclaw
ExecStart=/home/user/hydeclaw/hydeclaw-watchdog-aarch64 config/watchdog.toml
EnvironmentFile=/home/user/hydeclaw/.env
...
### hydeclaw-memory-worker.service
...
Type=notify
WorkingDirectory=/home/user/hydeclaw
ExecStart=/home/user/hydeclaw/hydeclaw-memory-worker-aarch64
EnvironmentFile=/home/user/hydeclaw/.env
WatchdogSec=300
Restart=always
RestartSec=10
...
```bash
# Статус
systemctl --user status hydeclaw-core
...
# Перезапуск
systemctl --user restart hydeclaw-core
...
> **docker/hydeclaw-core.service** в репозитории — устаревший пример для system-mode
> (`WantedBy=multi-user.target`). setup.sh создаёт user-mode юниты. Не использовать
> docker/hydeclaw-core.service напрямую.
...
|---|---|---|---|
| `postgres` | `hydeclaw-pg:17-age-pgvector` | `127.0.0.1:5432` | PostgreSQL 17 + pgvector + Apache AGE |
...
| `mcp-youtube-transcript` | 9043 | YouTube transcripts |
...
~/hydeclaw/scripts/mcp-deploy.sh stdio-node mcp/fetch:latest fetch 9040

# Python MCP через pip
~/hydeclaw/scripts/mcp-deploy.sh stdio-python mcp-server-git git 9012 mcp-server-git

# Внешний HTTP MCP
~/hydeclaw/scripts/mcp-deploy.sh url https://context7.com/mcp context7

# Удалить
~/hydeclaw/scripts/mcp-deploy.sh remove fetch
```
...
server {
    listen 80;
    server_name hydeclaw.local;

    # Static UI (Next.js out/)
    root /home/user/hydeclaw/ui/out;
    index index.html;
...
`/etc/nginx/sites-available/hydeclaw` или `/etc/nginx/conf.d/hydeclaw.conf`.
...
# Локально
curl -sf -H "Authorization: Bearer $HYDECLAW_AUTH_TOKEN" http://localhost:18789/api/doctor
...
HydeClaw не имеет встроенного rollback. Стратегия:

### Бинарники

```bash
# Перед обновлением (на Pi) сделать бэкап
cp ~/hydeclaw/hydeclaw-core-aarch64 ~/hydeclaw/hydeclaw-core-aarch64.bak

# Откат
cp ~/hydeclaw/hydeclaw-core-aarch64.bak ~/hydeclaw/hydeclaw-core-aarch64
systemctl --user restart hydeclaw-core
```
...
Удаляет:
- Все systemd-юниты `hydeclaw*.service`
- Все Docker контейнеры (compose + bollard-managed `hc-*`, `mcp-*`)
- Docker volume `pgdata` (все данные БД)
- Docker network `hydeclaw`
- Docker образы `hydeclaw-*`, `browser-renderer`, `searxng/*`
...
- Ротируй `HYDECLAW_AUTH_TOKEN` периодически
...
- Бэкапить `HYDECLAW_MASTER_KEY` отдельно
...
- Sensitive env vars (`HYDECLAW_*`, `DATABASE_URL`) фильтруются из окружения sandbox
...
- toolgate venv не создан: `ls ~/hydeclaw/toolgate/.venv`
...
```bash
systemctl --user restart hydeclaw-memory-worker
```
...
- Проверь subagents: `GET /api/tasks`
- Перезапусти memory worker: `systemctl --user restart hydeclaw-memory-worker`
...
# Watchdog
journalctl --user -u hydeclaw-watchdog -f

# Memory worker
journalctl --user -u hydeclaw-memory-worker -f

# Все сервисы
journalctl --user -u 'hydeclaw-*' -f
```
...
```
~/hydeclaw/
├── hydeclaw-core-aarch64         # main binary
├── hydeclaw-watchdog-aarch64     # watchdog binary
├── hydeclaw-memory-worker-aarch64# memory worker binary
├── .env                          # secrets (AUTH_TOKEN, MASTER_KEY, DATABASE_URL)
├── VERSION                       # текущая версия
├── setup.sh / update.sh / uninstall.sh
├── config/
│   ├── hydeclaw.toml             # основной конфиг
