...
//! должны иметь тег `@opex:ephemeral`, а тег `@opex:ephemeral` — отсутствовать.

use sqlx::PgPool;

/// Проверяет, что после миграций:
/// 1. Есть хотя бы одна таблица с тегом `@opex:ephemeral` (migration 063 отработала).
/// 2. Не осталось ни одной таблицы с тегом `@opex:ephemeral` (старый тег стёрт).
...
async fn ephemeral_tag_is_opex(pool: PgPool) {
    ...
    assert_eq!(
        legacy_count.0, 0,
        "не должно остаться тега @opex:ephemeral, найдено: {}",
        legacy_count.0
    );
}
