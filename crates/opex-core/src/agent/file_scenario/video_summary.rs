...
mod tests {
    ...
    fn build_reduce_messages_includes_all_partials_in_order() {
        ...
        // Polish constraints: hide chunking, no duplicate headings, consolidate Q&A.
        assert!(msgs[0].content.contains("НЕ упоминай"), "reduce hides chunking mechanics from output");
        assert!(msgs[0].content.contains("Вопросы и ответы"), "reduce consolidates Q&A");
        let user = &msgs[1].content;
