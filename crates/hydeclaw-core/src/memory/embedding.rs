...
/// `ToolgateEmbedder` implements it by delegating HTTP calls to `hydeclaw_embedding::ToolgateClient`.
use anyhow::Result;
...
/// Concrete embedder that delegates to `hydeclaw_embedding::ToolgateClient`.
///
...
impl ToolgateEmbedder {
    ...
    async fn do_initialize(&self) -> Result<()> {
        ...
        let existing_dim = hydeclaw_db::memory_queries::get_existing_embedding_dim(&self.db).await;
        ...
        if let Err(e) = hydeclaw_db::memory_queries::ensure_hnsw_index(&self.db, dim).await {
    ...
    async fn maybe_lazy_init_dim(&self, vec: &[f32]) {
            ...
            .is_ok()
        {
            ...
            if let Err(e) =
                hydeclaw_db::memory_queries::ensure_hnsw_index(&self.db, detected_dim).await
            {
