    ...
    if let Some(first) = detected.first() {
        enriched.push_str(&format!(
            "\n\n[Пользователь прислал ссылку, которую можно обработать: {}. \
             ...
             пользователю интерактивное меню выбора. НЕ выписывай и не пересказывай \
             список обработчиков сам (меню уже показано). Просто дождись выбора \
             пользователя и по нему вызови file_handler с action=\"run\", тем же \
             source_url и выбранным handler_id.]",
            detected.join(", "),
            first,
        ));
    }
