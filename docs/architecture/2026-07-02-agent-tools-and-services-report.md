...
Статус на 2026-07-03: **все пять пунктов реализованы** (hardening-серия коммитов):

- ~~Реализовать или удалить `_session_tool_state`~~ — удалён (`11f548d9`).
- ~~Скоупинг `penalty_score` по агенту~~ — сделан per `(agent_name, tool_name)`: миграция 070
  (новый PK), вложенный кэш `agent → tool → penalty`, тест `penalty_is_scoped_per_agent`
  (`b3578d9d`).
- ~~Хранить хэш аргументов в `session_timeline.tool_end`~~ — `args_hash` (hex от
  `loop_detector_key`) пишется в payload и восстанавливается `replay_from_timeline`
  (`5e5bcc72`).
- ~~Бюджет на цепочку decision-вебхуков~~ — `total_webhook_timeout_ms` (по умолчанию 10 с),
  режим `on_chain_timeout` (по умолчанию fail-open), сеттер `set_webhook_chain_budget`
  (`c66d591f`).
- ~~TTL на инструмент для семантического кэша~~ — секция `[semantic_cache]` с per-tool
  TTL/порогом сходства, подключена во всех точках `parallel.rs` (`29bc0189`).
