...
| `GET` | `/api/sessions/latest` | Получить последнюю сессию для агента (требует `?agent=`) |
...
| `POST` | `/api/sessions/{id}/compact` | Вручную сжать историю сессии (требует `?agent=`) |
| `GET` | `/api/sessions/{id}/export` | Экспортировать сессию в JSON или Markdown (требует `?agent=`) |
...
| `GET` | `/api/sessions/{id}/active-path` | Получить активный путь ветки сообщений (требует `?agent=`) |
...
| `PATCH` | `/api/messages/{id}` | Редактировать сообщение пользователя (требует `?agent=`) |
...
| Параметр | Тип | По умолчанию | Описание |
|----------|-----|--------------|----------|
| `agent` | string | — | Проверка владения |
| `format` | string | `json` | `json` или `markdown` |

Экспорт в Markdown возвращает `Content-Disposition: attachment; filename="session-{id}.md"`.
...
Редактировать сообщение пользователя (роль должна быть `user`). Требует параметр запроса `?agent=`.
...
`parent_message_id` и `branch_from_message_id` в сообщениях обеспечивают навигацию по дереву разговора. `POST /api/sessions/{id}/fork` создаёт новую ветку; `GET /api/sessions/{id}/active-path` возвращает текущий активный путь через дерево.
