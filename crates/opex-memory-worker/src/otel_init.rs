...
//! grouping. Lumping the worker under "opex-core" would hide which
...
pub fn init() {
    let env_filter = tracing_subscriber::EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| "opex_memory_worker=info".into());
    ...
    let service = std::env::var("OTEL_SERVICE_NAME")
        .unwrap_or_else(|_| "opex-memory-worker".to_string());
